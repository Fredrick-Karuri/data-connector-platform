  ...
  const columns = useMemo<ColumnDef<Row>[]>(() => {
    ...
    const dataColumns: ColumnDef<Row>[] = Object.keys(rows[0])
      ...
      .map((field) => ({
        ...
        cell: ({ row, getValue }) => {
          ...
          const rowId = String(
            rows[rowIdx]?.["id"] ?? rows[rowIdx]?.["_id"] ?? `row_${rowIdx}`,
          );
          const isDirty = Boolean(diffMap[rowId]?.[field]);
          const error = rowErrors[rowId]?.[field];
          return (
            <EditableCell
              rowId={rowId}
              field={field}
    ...
    dataColumns.push({
      ...
      cell: ({ row }) => {
        ...
        return (
          <button
            className={isDeleted ? g.deleteBtnOn : g.deleteBtn}
      ...
      <div className={g.toolbar}>
        <div className={g.toolbarLeft}>
          ...
          <span className={g.rowCount}>
            ...
            {dirtyCount > 0 && (
              <span className={g.dirtyBadge}>{dirtyCount} edited</span>
            )}
          </span>
        </div>
        <div className={g.toolbarRight}>
          {isDirty && (
            <button className={g.btnGhost} onClick={resetDiffs}>
              Reset
            </button>
          )}
          <select
            className={g.formatSelect}
            value={format}
